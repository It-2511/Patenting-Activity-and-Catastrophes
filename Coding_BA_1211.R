...
# Create a data frame with 8 data points for the population of the Connecticut counties since they are aggregated into Planning Regions in the country-wide population data. (These Planning Regions replace counties in their function as of 2022, however, the Catastrophe data is just organized into counties and therefore, I need the county population.) The data for Connecticut is taken from ct.gov, 2021 estimates.
Connecticut_population <- data.frame(
  County = c("Fairfield", "Hartford", "Litchfield", "Middlesex", "New Haven", "New London", "Tolland", "Windham"),
  Population_2022 = c(959768, 896854, 185000, 164759, 863700, 268605, 150293, 116418)
)
# Insert the new data into County_population after row 311
County_population <- rbind(
  County_population[1:309, ],Connecticut_population, County_population[310:nrow(County_population), ]
)
rm(Connecticut_population)
...
while (i < nrow(County_GDP)) { # continues as long as i is less than total row number
  if (all(is.na(County_GDP[i, ]))) { # the case of all NAs
    if (County_GDP[i + 1, "County"] != "District of Columbia") { # Keep DC 
...
County_GDP <- rbind(County_GDP[1:2902, ], Virginia_GDP, County_GDP[2903:nrow(County_GDP), ])
rm(Virginia_GDP)
...
Final_data_hazard <- Final_data_hazard %>%
  ...
  # Remove any catastrophes of the same size for the same county and year since I noticed a lot of duplicate entries with different Hazard types. I am not preferencing certain hazard types in any way. This is accomplished by using a filter because the repeated values aren't necessarily below one another.
  filter(row_number() == 1) %>%
...
model_1 <- feols(Patentcount ~ `Log-PropertyDmg` | Year + Quarter,
        data = fips_data_population_and_gdp,
        weights = ~ GDP_per_capita, 
        cluster = ~ FIPS)

summary(model_1)
# Estimate Year FE: 0.227, not significant
# Estimate Year + Quarter FE: 0.229, not significant
# Note: Log-PropertyDmg is likely not a good independent variable

# This performs the regression with PropertyDmg_per_GDP as the independent variable
model_2 <- feols(Patentcount ~ PropertyDmg_per_GDP | Year,
        data = fips_data_population_and_gdp,
        weights = ~ GDP_per_capita, 
        cluster = ~ FIPS)

summary(model_2)
# Estimate for Year FE: -6.7e-05, **
# Estimate for Year & Quarter FE: -6.7e-05, **

# This takes the above created variable Log-PropertyDmgPerCapita_manual as independent variable. 
model_3 <- feols(Patentcount ~ Log_PropertyDmgPerCapita_manual | Year + Quarter,
                 data = fips_data_population_and_gdp,
                 weights = ~ GDP_per_capita,
                 cluster = ~ FIPS)

summary(model_3)
# Estimate Year FE: -0.82, not significant
# Estimate Year & Quarter FE: -0.83, bot significant
...
fips_data_population_and_gdp <- fips_data_population_and_gdp %>%
  mutate(disaster_category = case_when(
    PropertyDmg_per_GDP == 0 ~ 1,  # No Disaster 
    PropertyDmg_per_GDP > 0 & PropertyDmg_per_GDP <= 0.01 ~ 2,  # Small Disaster (Between 0% and 1% of county-GDP)
    PropertyDmg_per_GDP > 0.01 & PropertyDmg_per_GDP <= 0.1 ~ 3,  # Medium Disaster (Between 1% and 10% of county-GDP)
    PropertyDmg_per_GDP > 0.1 & PropertyDmg_per_GDP <= 1 ~ 4,  # Large Disaster (Between 10% and 100% of GDP)
    PropertyDmg_per_GDP > 1 ~ 5  # Very Large Disaster (More than 100% of GDP)
  ))

# Display number of disasters per category
# Display the number of disasters in each bin without creating a new data frame
fips_data_population_and_gdp %>%
  count(disaster_category, name = "num_disasters") %>%
  print()
# 2: 1334
# 3: 4508
# 4: 9607
...
binned_model <- felm(
  Patentcount ~ factor(disaster_category) | FIPS + Year + Quarter,
  data = fips_data_population_and_gdp
)

summary(binned_model)
# Estimates with just Year and FIPS as FE
# Estimate cat. 2: 26.5885 ***
# Estimate cat. 3: 5.2904 **
# Estimate cat. 4: -0.11
# Estimate cat. 5: -0.90 **
# Estimates with Quarter as FE are very similar
...
simple_panel_model <- felm(
  Patentcount ~ disaster_category | FIPS + Year,
  data = fips_data_population_and_gdp
)

summary(simple_panel_model)
# Estimate: -0.04406, not significant. Similar including Quarter as FE
...
# Regression 2 : Run the panel regression with fixed effects for FIPS and Year via index
...
panel_model_elevated <- plm(
  Patentcount ~ factor(disaster_category),
  data = fips_data_population_and_gdp,
  index = c("FIPS", "Year"),
  model = "within"
)
...
# Estimate using FIPS and Year as FE:
# Bin 2: 24.508 ***
# Bin 3: 9.060 ***
# Bin 4: 3.322 ***
# Bin 5: 2.147 ***
